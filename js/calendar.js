...
for (let i = 0; i < document.querySelectorAll(".price-tag").length; i++) {
    ...
    document.querySelectorAll(".price-tag")[i].textContent = `${prices[i]}лв.`
    document.querySelectorAll(".per-person")[i].textContent = `по ${Math.round(prices[i]/(minimumPeople+i))} на човек`

    document.querySelector(".big").textContent = `${Math.round(prices.slice(-1)[0] / (prices.length + divider))}лв.`;
}
